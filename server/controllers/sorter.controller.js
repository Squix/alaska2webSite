...
class SorterController {
  ...
  static sort = async (req, res) => {
    ...
    if (req.query["usm_on"] || req.query["usm_off"]) {
      ...
      if (
        typeof req.query["usm_on"] !== "undefined" &&
        req.query["usm_on"] == "true"
      ) {
        usm_array.push("1");
      }
      if (
        typeof req.query["usm_off"] !== "undefined" &&
        req.query["usm_off"] == "true"
      ) {
        usm_array.push("0");
      }
    ...
    db = await csvdb(
      process.cwd() + "/server/public/" + process.env.DB_FILE_NAME,
      {
        delimiter: ";",
        quote: false,
        relax: true,
    ...
    let records = await db.find(function (record) {
      ...
      for (const criteria in criteriaObject) {
        ...
        if (
          criteriaObject[criteria].type !== "range" &&
          value.split(",").indexOf(record[criteria]) === -1
        ) {
        ...
        //si c'est un inputRange
          ...
          if (
            parseFloat(record[criteria]) < range.min ||
            parseFloat(record[criteria]) > range.max
          ) {
            matchingRecord = false;
          }
        }
