...
const sendConfirmationEmail = (user) => {
  return mailHelper.send(
    ...
    `email confirmation`,
    `Hello !
    It seems that you requested an account on the ALASKA contest on steganography website https://alaska.utt.fr
    If you did not make such a request, simply ignore this email and accept our apologies.
    Otherwise, you have one last step to get onboard; click on the following link to enable your account on ALASKA website contest:
    ${process.env.SERVER_ROOT}?token=${user.verificationToken}
    We hope you'll have fun with us.
    
    Don't forget that you can also get some news from the ALASKA contest through the blog on which we will keep posting from news.
    From the ALASKA organization team 
    Remi COGRANNE, Quentin GIBOULOT and Patrick BAS.
    
    PS : In the (very) unlikely case of trouble of any kind, gettng the data, providing answers our using the script available, do not hesitate to shout us an email either to alaska@utt.fr or to remi.cogranne@utt.fr
    `
  )
}
      ...
      user.save()
        .then(user => {
          return mailHelper.send(
            ...
            `reset password`,
	    `Hello !
	    It seems that you requested to change the password associated with your account on ALASKA contest on steganography website https://alaska.utt.fr
	    To do so, click on the following link and you'll be all set:
